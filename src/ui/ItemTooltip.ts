...
export class ItemTooltip {
  ...
  show(item: ItemDefinition, ax: number, ay: number, side: 'left' | 'right' = 'right'): void {
    ...
    line(item.name, item.setId ? SET_COLOR : numHex(RARITY_COLOR[item.rarity] ?? 0xffffff), 13, false, true);
    ...
    // class armor set block: set name + what each tier unlocks
  ...
  showText(title: string, body: string, ax: number, ay: number, side: 'left' | 'right' = 'right'): void {
    ...
    let y = PAD;
    let maxW = 0;
    const line = (str: string, color: string, size: number, wrap = true): void => {
      const t = this.scene.add
        .text(0, 0, str, {
          fontFamily: SERIF,
          fontSize: `${size}px`,
